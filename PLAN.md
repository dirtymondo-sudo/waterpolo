...
- **Input abstraction** supporting keyboard+mouse and gamepad from day one (the 2K feel wants a controller).
  - Gamepad layer lives in `src/input/gamepad.js`: a pure poll that turns any
    `Gamepad` into a semantic snapshot. First-class support for the **Nintendo
    Switch Pro Controller over USB** — buttons are mapped by Nintendo label (so
    on-screen prompts match the engraving), with both `mapping:"standard"` and a
    best-effort raw-HID fallback handled, radial stick deadzone, and D-pad
    (buttons or hat axis). Default scheme: L-stick/D-pad swim, ZR/ZL sprint,
    **A** shoot, **X** skip, **Y** lob, **L** pass, **R** switch/steal, **−**
    camera. Dual-rumble (`InputManager.rumble`) fires on goals and shot release;
    a HUD toast announces connect/disconnect.
